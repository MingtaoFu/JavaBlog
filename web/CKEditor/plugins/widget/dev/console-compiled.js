...
(function () {
	...
	CKCONSOLE.add('widget', {
		panels: [{
			...
			type: 'box',
			content: '<ul class="ckconsole_list">' + '<li>focused: <span class="ckconsole_value" data-value="focused"></span></li>' + '<li>selected: <span class="ckconsole_value" data-value="selected"></span></li>' + '</ul>',
			...
			refresh: function (editor) {
				...
				for (var i = 0; i < selected.length; ++i) selectedIds.push(selected[i].id);
			...
			on: function (editor, log, logFn) {
				...
				editor.on('selectionChange', function (evt) {
					...
					if (el && (widget = editor.widgets.getByElement(el, true))) msg += ' (id: ' + widget.id + ')';
